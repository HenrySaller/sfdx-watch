'use strict';

const fs = require('fs');
const path = require('path');
const colors = require('ansi-colors');
const prettyTime = require('pretty-hrtime');
const gulp = require('gulp');
const tap = require('gulp-tap');
const sass = require('gulp-sass');
const spawn = require('child_process').spawn;

function logError(message, error) {
  return console.log(`\n${colors.red('Error')} ${message}.\n\n${error}`);
}

function Reporter() {
  let triggered = false;
  function report() {
    if (!triggered) {
      logError(
        'reading "sfdx-project" config',
        'Make sure that "sfdx-project.json" exists and is propperly configured.\n' +
        'Assuming that a default "force-app" project is used.\n'
      );
      triggered = true;
    }
  }

  return { log: report }
}

function File(path, project) {
  this.path = path;
  this.exists = fs.existsSync(path);
  this.defaultPackage = project;
  this.reporter = new Reporter();
}

const sfdxConfig = new File('./sfdx-project.json', 'force-app');

function getProjectPath(ext) {
  const config = sfdxConfig.exists && JSON.parse(fs.readFileSync(sfdxConfig.path, 'utf8'));
  const packageDirectories = config && config.packageDirectories;
  const withExt = (path) => './' + path + (ext ? '/**/*.' + ext : '/**/*');

  if (config && packageDirectories) {
    const packages = packageDirectories.map(pack => pack.path);
    const paths = packages.map(withExt);

    return paths;
  } else {
    sfdxConfig.reporter.log();

    return withExt(sfdxConfig.defaultPackage);
  }
}

function formatError(e) {
  if (!e.error) { return e.message; }
  if (typeof e.error.showStack === 'boolean') { return e.error.toString(); }
  if (e.error.stack) { return e.error.stack; }
  return new Error(String(e.error)).stack;
}

function logEvents(task) {
  console.log(`\n > Starting \'${colors.cyan(task)}\' task`);

  gulp.on('start', (evt) => {
    console.log(` > Starting \'${colors.cyan(evt.name)}\'...`);
  });

  gulp.on('stop', (evt) => {
    const time = prettyTime(evt.duration);
    console.log(` > Finished \'${colors.cyan(evt.name)}\' after ${colors.magenta(time)}`);
  });

  gulp.on('error', (evt) => {
    const msg = formatError(evt);
    const time = prettyTime(evt.duration);
    console.log(` > \'${colors.cyan(evt.name)}\' ${colors.red('errored')} after ${colors.magenta(time)}`);
    console.log(`\n${msg}\n`);
  });
}

function scss() {
  return gulp.src(getProjectPath('scss'))
    .pipe(sass.sync().on('error', function(err) {
      logError('during "scss" compile', err.formatted);
      this.emit('end');
    }))
    .pipe(tap(file => file.contents = Buffer.concat(
      [
        Buffer.from(
          `/* Do not edit this file directly.\n` +
          `   Please use ${path.basename(file.path, '.css')}.scss instead. */\n\n`
        ), file.contents
      ]
    )))
    .pipe(gulp.dest(file => file.base));
}

function deploy(exit) {
  const cp = spawn('sfdx', ['force:source:push']);
  const log = [];

  cp.stdout.on('data', data => log.push(data));
  cp.stderr.on('data', data => log.push(`Error: ${data}`));

  cp.on('close', (code) => (code !== 1) && console.log('\n' + log.join('')));
  cp.on('exit', (code) => {
    if (code === 1) {
      logError('during "sfdx" deploy', log.join(''));
      exit();
    }
  });

  return cp;
}

function watch(logEnabled) {
  if (logEnabled) logEvents('watch');

  gulp.watch(getProjectPath('scss'), scss);
  gulp.watch(getProjectPath('!(scss)'), deploy);
}

exports.watch = watch;
